...
describe('GET /hotels', () => {
  ...
  describe('when token is valid', () => {
    it("should respond with status 404 when user doesn't have an enrollment", async () => {
    ...
    it("should respond with status 402 if ticket isn't paid yet", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
    ...
    it("should respond with status 402 if hotel isn't included", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
...
describe('GET /hotels/:hotelId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it("should respond with status 404 if hotel isn't found", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithParams(false, true);
